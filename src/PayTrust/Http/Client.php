...
namespace PayTrust\Http;

use GuzzleHttp\Client as GuzzleClient;
use Psr\Http\Message\ResponseInterface;
use PayTrust\Exceptions\BadRequest;
use PayTrust\Exceptions\InvalidArgument;
...
class Client
{
    ...
    /** @var string */
    private $user_agent = "PayTrust_PHP/1.0.0-rc.1 (https://github.com/webguru221/paytrust-php)";
    ...
    public function __construct($config = [], $client = null, $clientOptions = [], $wrapResponse = true)
    {
        ...
        $this->key = isset($config["key"]) ? $config["key"] : getenv("PayTrust88_SECRET");
        if (empty($this->key)) {
            throw new InvalidArgument("You must provide a PayTrust88 api key or token.");
        }
     ...
     * @param  string $endpoint The PayTrust88 API endpoint
     ...
     * @return \PayTrust\Http\Response|ResponseInterface
     * @throws \PayTrust\Exceptions\BadRequest
     */
